    ...
    with executor.batch():  # job array
        for env_name in ["cube-octuple-play-oraclerep-v0"]:
            for dataset_name in ["cube-octuple-play-100m-v0"]:
                for alpha_critic in [1, 3, 10]:
                    for alpha_actor in [100, 300, 1000]:
                        for critic_loss_type in ['sarsa', 'q-learning']:
                            for value_layer_norm in [True]:
                                for actor_layer_norm in [True]:
                                    for seed in [10, 20]:
                                        exp_name = f"{datetime.today().strftime('%Y%m%d')}_fdrl_{env_name}_{dataset_name}_alpha_critic={alpha_critic}_alpha_actor={alpha_actor}_critic_loss_type={critic_loss_type}_value_layer_norm={value_layer_norm}_actor_layer_norm={actor_layer_norm}"
                                        log_dir = os.path.expanduser(
                                            f"{log_root_dir}/exp_logs/horizon_reduction_logs/fdrl/{exp_name}/{seed}")
