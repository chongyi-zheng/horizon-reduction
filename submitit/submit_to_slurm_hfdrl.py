                        ...
                        for critic_loss_type in ['sarsa']:
                            for value_layer_norm in [True]:
                                for actor_layer_norm in [True]:
                                    for seed in [10, 20]:
                                        exp_name = f"{datetime.today().strftime('%Y%m%d')}_hfdrl_{env_name}_{dataset_name}_alpha_critic={alpha_critic}_num_samples={num_samples}_critic_loss_type={critic_loss_type}_value_layer_norm={value_layer_norm}_actor_layer_norm={actor_layer_norm}"
                                        log_dir = os.path.expanduser(
                                            f"{log_root_dir}/exp_logs/horizon_reduction_logs/hfdrl/{exp_name}/{seed}")
